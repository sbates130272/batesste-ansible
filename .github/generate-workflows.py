...
def generate_workflow(role_name: str, config: Dict) -> Dict:
    ...
    if len(config.get("ubuntu_versions", [])) > 1:
        ...
        if "26.04" in config["ubuntu_versions"] and not config.get(
            "ignore_failure", False
        ):
            workflow["jobs"][f"{role_name}-test"][
                "continue-on-error"
...
def parse_args() -> argparse.Namespace:
    ...
    default_workflows = script_dir / "workflows"

    parser = argparse.ArgumentParser(
        description="Generate GitHub Actions workflow files for each "
        "Ansible role with tests."
    )
    parser.add_argument(
        "--roles-dir",
        type=Path,
        default=None,
        help="Directory containing Ansible roles (default: <repo>/roles)",
    )
    parser.add_argument(
        "--workflows-dir",
        type=Path,
        default=None,
        help="Directory to write workflow files (default: .github/workflows)",
    )
    parser.add_argument(
        "-n",
        "--dry-run",
        action="store_true",
        help="Do not write files; only list roles and output paths",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        metavar="VERSION",
        default=None,
        help="Ubuntu versions for workflow matrix (e.g. 24.04 26.04). "
        "Overrides DEFAULT_CONFIG and ROLE_CONFIGS for this run.",
    )
    args = parser.parse_args()

    if args.roles_dir is None:
        args.roles_dir = repo_root / "roles"
    if args.workflows_dir is None:
        args.workflows_dir = default_workflows
...
def main() -> None:
    ...
    if args.verbose:
        ...
        if args.ubuntu_versions is not None:
            print(f"Ubuntu versions (override): {args.ubuntu_versions}")
    ...
    if args.dry_run:
        ...
        for role_name in roles:
            print(f"  {workflows_dir / f'{role_name}-ci.yml'}")
        return

    workflows_dir.mkdir(parents=True, exist_ok=True)

    for role_name in roles:
        config = {**DEFAULT_CONFIG, **ROLE_CONFIGS.get(role_name, {})}
        if args.ubuntu_versions is not None:
