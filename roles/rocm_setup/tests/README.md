...
```bash
cd roles/rocm_setup
ansible-playbook -i ../../inventory/inventories/hosts-rocm-setup.yml ./tests/test.yml
```
...
```bash
cd roles/rocm_setup
ansible-playbook -i ../../inventory/inventories/hosts-rocm-setup.yml ./tests/test.yml -e "rocm_setup_install_metrics_exporter=false"
```
